...
public class WorkImageDaoImpl implements IWorkImageDao {
	...
	public DaoResult getAllWorkImageDao(WorkImage workImage) {
		String sql="SELECT work_image_path "
				+ "FROM work_image "
				+ "WHERE work_id = ?";
		List<WorkImage> theResult = template.query(sql, new Object[] {workImage.getWork().getWorkId()}, (rs,row)->new WorkImage.Builder().workImagePath(rs.getString("work_image_path")).build());
